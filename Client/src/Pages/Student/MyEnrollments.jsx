...
const MyEnrollments = () => {
  const { calculateCourseDuration, navigate, backend, getToken, calculateNoofLectures } = useContext(AppContext)
        ...
        courses.map(async (course) => {
          try {
            const { data } = await axios.post(
              `${backend}/api/user/get-course-progress`,
              { courseId: course._id },
              { headers: { Authorization: `Bearer ${token}` } }
            )
              ...
              <tr key={index} className='border-b border-gray-500/20'>
                <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                  ...
                  <div className='flex-1'>
                    ...
                    <Line
                      strokeWidth={2}
                      percent={progressArray[index] ? (progressArray[index].lecturesCompleted / progressArray[index].totalLectures) * 100 : 0}
